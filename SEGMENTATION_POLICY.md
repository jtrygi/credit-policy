...
`images/frontier_comparison.png` makes this visible directly rather than at two isolated table points: it overlays the baseline grade frontier (dashed grey, 7 discrete cutoffs) against each model's full continuous approve-lowest-PD-first frontier on test. Both model curves sit below baseline on loss rate and above it on profit **across the whole range**, not just at the reference point -- and XGBoost (purple) is visibly ahead of LR-24 (blue) on both, the interpretability-cost tradeoff in one picture.
...
`images/baseline_policy_curve.{png,csv}`, `images/baseline_policy_reference.json`, `images/val_scores.csv`, `images/test_scores.csv`, `images/segments_{lr24,xgb}.png`, `images/segments_{lr24,xgb}_{val,test}.csv`, `images/segment_policy_results.json`, `images/frontier_comparison.png`. Models: `models/lr-forward-selected-24/v1/` (newly persisted).
